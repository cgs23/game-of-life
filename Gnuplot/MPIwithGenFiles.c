...
{
    int dimension, generation, rank, numOfProcesses, blockDimension, fileValue, i, j, w, z, x, y, m, s, rankId;
    int right, left, up, down, upRight, upLeft, downRight, downLeft;
    int alive = 0, different = 0, neighborsForRows, neighborsForColumns, neighbors;
    int sumAlive, sumDifferent, **temp;
    int UP = 0, DOWN = 1, LEFT = 2, RIGHT = 3;
    int UP_LEFT = 4, UP_RIGHT = 5, DOWN_LEFT = 6, DOWN_RIGHT = 7;
    double start_time, end_time;
    FILE *inputFile, *genFile;
    ...
    int** genForFile = malloc((blockDimension + 2)*sizeof(int*));
    ...
    genForFile[0] = malloc((blockDimension + 2)*(blockDimension + 2)*sizeof(int));

	for (i = 1; i < blockDimension + 2; i++) {
		currentGen[i] = &(currentGen[0][i*(blockDimension + 2)]);
		newGen[i]  = &(newGen[0][i*(blockDimension + 2)]);
        genForFile[i]  = &(genForFile[0][i*(blockDimension + 2)]);
	}
    ...
    //     printf("the upLeft is %d\n", upLeft);
    //     printf("the upRight is %d\n", upRight);
    //     printf("the downLeft is %d\n", downLeft);
    //     printf("the downRight is %d\n", downRight);
    // }
    ...
    for(i=0; i < generation; i++)
    {
        ...
        for (x = 2; x < blockDimension; x++) {
            for (y = 2; y < blockDimension; y++) {
                ...
                // if(i == 2)
                // {
                //     printf("The number of neighbors for (%d,%d) is %d for rank %d\n", x,y,neighbors, rank);
                // }
        ...
        for(x = 1; x < blockDimension + 1; x++){
            ...
            // if(i == 2)
            // {
            //    printf("The number of neighbors for (%d,%d) is %d for rank %d\n", blockDimension,x,neighbors, rank);
            // }
        ...
        for(x = 1; x < blockDimension + 1; x++){
            ...
            // if(i == 2)
            // {
            //     printf("The number of neighbors for (%d,%d) is %d for rank %d\n", x,blockDimension,neighbors, rank);
            // }
        ...
        /* START - send and receive for GNUPLOT */
        if(rank != 0)
        {
            MPI_Send(&(currentGen[1][1]), (blockDimension + 1) * (blockDimension + 1), MPI_INT, 0, 0,MPI_COMM_WORLD);
        }
        else
        {
            snprintf(fileNameBuffer, sizeof(char) * 32, "generation%i.txt", i + 1);
            genFile = fopen(fileNameBuffer, "w+");
            for(m = 1; m < blockDimension + 1; m++)
            {
                for(s=1; s<blockDimension + 1; s++)
                {
                    fprintf(genFile, "%d\n", currentGen[m][s]);
                }
            }
            for(rankId = 1; rankId < numOfProcesses; rankId++)
            {
                MPI_Recv(&(genForFile[1][1]), (blockDimension + 1) * (blockDimension + 1), MPI_INT, rankId, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                for (m = 1; m < blockDimension + 1; m++) {
                    for (s = 1; s < blockDimension + 1; s++) {
                        fprintf(genFile, "%d\n", genForFile[m][s]);
                    }
        ...
        // printf("For gen %d and rank %d the alive is %d and different is %d\n", i, rank, alive, different);
        /* END - send and receive for GNUPLOT */
